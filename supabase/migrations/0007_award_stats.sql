-- Ладдер страждання — три нові колонки статистики для спецнагород
-- лідерборду: агресія (середня ставка на спробу), поїздки в нуль і
-- кількість платних каменів.
-- Виконати один раз: Dashboard → SQL Editor → New query → вставити весь
-- файл → Run. (Замінює функцію тригера з 0006 — сам тригер лишається.)
--
-- ВІДМІННІСТЬ від моделі 0005/0006: ці колонки сервер НЕ звіряє з
-- надісланими значеннями, а САМ обчислює з history і перезаписує.
-- Дві причини:
--  1) Сумісність: старий клієнт цих полів не шле — точна звірка означала б
--     "усі сабміти відхилено" між прогоном міграції і деплоєм клієнта.
--     А так значення завжди заповнює сервер — байдуже, що прийшло.
--  2) Безпека: перезапис — це строго сильніше за валідацію (підробити
--     значення неможливо в принципі, воно ігнорується).
-- Для адміна (merge/rename) обчислення пропускається — зберігаються
-- значення з рядка-джерела (у legacy-записів без history вони й так 0).

alter table ladder_entries add column if not exists aggression int not null default 0;
alter table ladder_entries add column if not exists times_hit_zero int not null default 0;
alter table ladder_entries add column if not exists paid_attempts int not null default 0;
...
begin
  ...
  for idx in 0 .. n - 1 loop
    ...
    -- mirage/sky ризикують усім рівнем, under — одним, world — нічим.
  ...
  -- перетворювала чесний сабміт на "читерський".
  ...
  -- (дзеркало aggression у src/lib/rngProfile.ts: середня ставка / 1.5).
...
-- =========================================================
-- Одноразовий бекфіл: рахуємо нові колонки з уже збереженої history для
-- наявних записів (інакше вони з нулями не змагаються за нові нагороди,
-- поки гравець не переграє). Тригер на час бекфілу вимикаємо — його
-- перевірка "строго кращий" інакше відхилила б ці UPDATE.
-- Записи без history (створені до 0005) лишаються з нулями — чесних даних
-- для них просто немає.
-- =========================================================
alter table ladder_entries disable trigger ladder_entries_validate_trg;

update ladder_entries e set
  aggression = calc.aggression,
  times_hit_zero = calc.hit_zero,
  paid_attempts = calc.paid
from (
  select
    nickname,
    greatest(0, least(100, round(((coalesce(sum(case
      when elem ->> 'method' in ('mirage', 'sky') then (elem ->> 'before')::int
      when elem ->> 'method' = 'under' then least(1, (elem ->> 'before')::int)
      else 0
    end), 0)::numeric / greatest(count(*), 1)) / 1.5) * 100)::int)) as aggression,
    count(*) filter (
      where (elem ->> 'success')::boolean = false
        and (elem ->> 'after')::int = 0
        and (elem ->> 'before')::int >= 1
    ) as hit_zero,
    count(*) filter (where elem ->> 'method' <> 'mirage') as paid
  from ladder_entries, jsonb_array_elements(history) as elem
  group by nickname
) calc
where e.nickname = calc.nickname;

alter table ladder_entries enable trigger ladder_entries_validate_trg;
