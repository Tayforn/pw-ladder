-- Ладдер страждання — нова економіка РЕСУРСІВ замість балів.
-- Виконати один раз: Dashboard → SQL Editor → New query → вставити весь
-- файл → Run. (Замінює функцію тригера з 0008 — сам тригер лишається.)
--
-- Модель:
--  * МІРАЖ = СПРОБА: кожна спроба будь-яким методом споживає 1 міраж;
--    спроба каменем додатково споживає 1 одиницю свого каменя. Тож
--    attempts (довжина history) == спожиті міражі.
--  * Ліміти видає адмінка (ladder_settings): mirage_count / sky_count /
--    under_count / world_count + decoy_count (кількість підставних СЛОТІВ,
--    кожен зі своїм рівнем; слоти предметів: 'a' + до 5 підставних 'b'..'f').
--  * Бали (points) більше не грають: клієнт шле 0, клемп прибрано.
--    Колонки старої економіки (points_per_success тощо) лишаються в таблиці
--    незайманими — на випадок відкату.
--  * Рейтинг: level DESC → attempts ASC → paid_attempts ASC (менше платних
--    каменів — краще). paid_attempts уже рахує сервер (0007).
--  * Валідація лімітів — ЖОРСТКА З ЗАПАСОМ ×1.5 від поточних налаштувань
--    (запас — щоб зміна лімітів адміном посеред чийогось забігу не робила
--    чесний сабміт "читерським"). Плюс абсолютний кап 500 спроб.
--
-- УВАГА (перехід): забіги, початі за старої бальної економіки, могли
-- використати більше каменів, ніж нові ліміти ×1.5 — такі сабміти після
-- цієї міграції буде відхилено. Онови ліміти в адмінці під реальні цифри
-- або оголоси "новий сезон" (обнуління ладдера).

alter table ladder_settings add column if not exists mirage_count int not null default 200;
alter table ladder_settings add column if not exists sky_count int not null default 15;
alter table ladder_settings add column if not exists under_count int not null default 15;
alter table ladder_settings add column if not exists world_count int not null default 30;
alter table ladder_settings add column if not exists decoy_count int not null default 1;
...
declare
  ...
  world_n int := 0;
  cfg record;
  -- камбек переможця (другий прохід)
...
begin
  ...
  -- Адміну (merge/rename/ручні правки) валідація не потрібна.
  if is_admin() then
    return new;
  end if;
  ...
  -- Прохід 1: перевіряємо кожен перехід (у ланцюжку СВОГО слота) і
  ...
  for idx in 0 .. n - 1 loop
    ...
    -- Послідовність (усі предмети разом)
    if cur_success then
  ...
  -- Прохід 2: ланцюжок переможця → "найбільший відкат".
  for idx in 0 .. n - 1 loop
    elem := new.history -> idx;
  ...
  -- щоб зміна лімітів адміном посеред чийогось забігу не зробила чесний
  -- сабміт "читерським". attempts == спожиті міражі (міраж = спроба).
  ...
  -- Бали більше не грають: нормалізуємо в 0 (колонка лишається для історії).
  new.points := 0;
